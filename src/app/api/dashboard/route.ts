...
export async function GET() {
  ...
  const [leadsCountRes, apptTodayRes, apptTomRes, allLeadsRes] = await Promise.all([
    ...
    // Appointments today (with linked lead name)
    supabase
      .from('appointments')
    ...
    // All leads for pipeline widget + pipeline value
    supabase
      .from('leads')
  ...
  // Pipeline value = sum of budget_max for non-completed leads
  const pipelineValue = allLeads
    .filter(l => l.status !== 'realizado')
    .reduce((sum, l) => sum + ((l.budget_max as number | null) ?? 0), 0)

  // Group leads into the 4 dashboard columns
